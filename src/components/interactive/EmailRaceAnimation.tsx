...
export function EmailRaceAnimation() {
  ...
  // Restart the loop whenever the component comes into view
  useEffect(() => {
    if (!isInView) return;
  ...
  // Each tick remounts the inner tree, restarting all animations
    ...
    <div ref={ref} className="w-full max-w-3xl mx-auto">
      {isInView && (
        <motion.div
          ...
          transition={{ duration: CYCLE, times: [0, 0.03, 0.86, 0.9] }}
          className="space-y-6"
        >
          {/* ── Header ──────────────────────────────────────────── */}
